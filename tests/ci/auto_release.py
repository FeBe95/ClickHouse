...
def _read_cmake_version_field(branch: str, field: str) -> str:
    """Return the value of `SET(VERSION_<field> ...)` from cmake/autogenerated_versions.txt
    at the tip of `origin/<branch>`. The post-release version bump on a release branch
    rewrites this file, so VERSION_GITHASH and VERSION_DESCRIBE there always identify
    the most recent release on the branch — without having to sort tag refs (which
    GitHub returns lexically, treating v26.3.9.8-lts as newer than v26.3.10.62-lts).
    """
    content = Shell.get_output_or_raise(
        f"git show origin/{branch}:cmake/autogenerated_versions.txt"
    )
    match = re.search(rf"SET\(VERSION_{field}\s+(\S+)\)", content)
    if not match:
        raise RuntimeError(
            f"VERSION_{field} not found in cmake/autogenerated_versions.txt at origin/{branch}"
        )
...
def _prepare(token):
    ...
    for pr in prs:
        ...
        # Source of truth for "the previous release on this branch" is the cmake file
        # rewritten by the post-release version-bump commit, not the tag list. This
        # avoids being fooled by tag-ref lexical ordering (`v26.3.10.62-lts` vs
        # `v26.3.9.8-lts`) and never picks a release commit that has already shipped.
        # VERSION_PATCH is bumped by every patch release, so `PATCH == 1` reliably
        # identifies a brand-new branch that hasn't had its first release yet.
        if int(_read_cmake_version_field(pr.head.ref, "PATCH")) == 1:
            print("It's a new release branch - skip auto release for it")
            continue
        previous_release_describe = _read_cmake_version_field(pr.head.ref, "DESCRIBE")
        previous_release_sha = _read_cmake_version_field(pr.head.ref, "GITHASH")

        commits = Shell.get_output_or_raise(
            f"git rev-list --first-parent {previous_release_sha}..origin/{pr.head.ref}",
        ).split("\n")
        commit_num = len(commits)

        print(
            f"Previous release [{previous_release_describe}] at [{previous_release_sha}] — "
            f"[{commit_num}] commits to branch tip"
        )
